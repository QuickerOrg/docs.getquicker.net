...
默认使用全局设置中的位置；未修改全局设置时出现在屏幕底部居中。
...
**显示位置**：选择「默认位置（使用全局设置）」，或固定在屏幕六个位置之一。仅影响成功 / 信息 / 告警 / 错误提示，不控制 Windows 通知。
...
**保持秒数**：`0` 用默认时长，`-1` 一直显示，或填秒数。2.2.17 起按设置可保持 **1 秒**；更早版本界面与元数据常见范围为 2～30 秒。

**显示时间**：有标题时在右上角显示发出时间；无标题的紧凑条不显示。

### 全局默认位置和时长

2.2.22 起，在 **设置 → 模块功能选项 → 提示消息** 中，可设置默认显示位置和默认停留时间，保存后随账号同步到其他设备。

- 默认位置可选底部居中、左下角、右下角、顶部居中、左上角和右上角。
- 默认停留时间为 `0` 时，根据文本长度自动计算；也可设为 1～30 秒。采用默认时长且带操作按钮的提示至少显示 10 秒。
- 步骤的「显示位置」选择「默认位置（使用全局设置）」时跟随全局设置；选择具体位置时使用该位置。
- 步骤的「保持秒数」为 `0` 时采用默认时长；明确填写秒数或 `-1` 时使用该步骤的设置。

升级前保存的旧「底部居中」选项在本版对应「默认位置（使用全局设置）」。没有改动全局位置时，外观仍为底部居中；需要始终固定在底部时，请重新选择本版的「底部居中」。

回退旧客户端后，保存设置并同步可能重置新增的默认位置和时长；建议先记录所选值，并在所有设备升级后再修改。详见[升级注意](/important-notice#2222-提示设置与编辑操作)。
