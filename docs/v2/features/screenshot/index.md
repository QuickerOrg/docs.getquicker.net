...
截图界面内已集成 OCR、图片翻译、智能马赛克、斜铺文字水印等常用后处理。2.1.27 起，快速截图选区可正确命中部分透明顶层弹窗，贴图窗口已显示后也不再误弹“未产生位图”。2.1.28 起，截图 Pro 可用 **套索裁切** 自由圈选保留区域；贴图右键菜单的「图像处理」可用 **抠出主体（透明背景）**，2.1.29 起可手动选择 **通用抠图** 或 **人像抠图**，在本机抠图后继续保存或贴图。更复杂的自动化仍可在动作链中用独立步骤完成。截图历史、已关闭贴图与录屏成品可在采集历史中心统一查看与管理；2.1.23 起，进行中的录屏也可以在这里管理，并可在页内预览、复制、另存和切换播放速度。
