...
2.2.7 起，若某个字段绑定了不存在的变量，Quicker 会在打开表单前明确提示缺失变量，而不是进入表单后再异常中断。2.2.8 起，多字段表单中对小数连续加减时，不再出现多余尾数。2.2.13 起，可使用单选按钮列表，并为文本字段启用表达式校验。2.2.14 起，常用扩展选项可直接在结构化界面编辑，仍保留高级原始扩展设置入口；校验规则统一在同一输入框中设置。2.2.15 起，单选列表与字段标题的对齐已修复。
...
2.2.22 起，多选下拉框选中后的标签也会显示选项图标，便于与下拉列表中的图标和名称对应。
