...
2.x 的普通运行模式改用 Quicker 自带的 Roslyn 编译链，不再依赖 1.x 使用的旧脚本组件，并提供与当前运行环境一致的代码补全和诊断。2.2.21 起补全默认走在线服务，也可在「设置 → 动作设计」安装并启用本地补全组件，见 [代码补全服务](/v2/xaction/concepts/xaction-editor#代码补全服务)。

除了继续支持完整的 `Exec(context)` 方法，现在也可以直接编写顶层脚本，通过 `context` 读写动作变量，并把末行表达式作为返回值。脚本返回 `Task` 或 `ValueTask` 时，Quicker 会等待完成并输出解包后的结果。

2.1.26 起，代码编辑器支持 `#region` 和 `//#region` 区域折叠，便于把较长脚本分段；普通模式也继续兼容旧式末行表达式，旧脚本通常不需要为返回值单独改写。
