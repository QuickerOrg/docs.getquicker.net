...
quickerDocKey: "xaction/module/sys:revitcontrol"
comments: true
...
使用前先[安装并连接 Revit Bridge](/v2/features/software-connections/software/revit)。本模块用于文档、元素和参数、选择与视图、文件导出及脚本；根据任务选择对应操作，或发送当前实例支持的自定义命令。

## 当前模块定义

<XActionModuleMeta moduleKey="sys:revitcontrol" />
...
文档、元素和参数、选择与视图、文件导出及脚本是常见使用方向。选择操作后填写对应字段；字段名称、默认值、枚举和条件以上方当前模块定义为准。
...
回到已连接的 Revit 窗口，再通过面板或配置好的快捷键触发动作。
...
- [Revit 安装与使用指南](/v2/features/software-connections/software/revit)
