...
**添加项子程序** / **编辑项子程序**（高级，2.2.22+）：可选。留空时仍用内置文本框或「编辑菜单项」窗口。填写子程序标识后，添加或编辑会调用对应子程序，便于用[多字段表单](/v2/xaction/modules/form)处理 JSON 等复杂项。当前 `data/xaction` 尚未同步这两项的元数据预览；步骤编辑器里可在高级参数中看到完整说明，也以客户端为准。

## 用子程序自定义添加和编辑

2.2.22 起，可在高级参数中指定 **添加项子程序**、**编辑项子程序**（也可填同一个子程序）。不配置时行为与旧版相同。

子程序与列表窗口约定下列变量（区分大小写）：

| 变量 | 方向 | 含义 |
| --- | --- | --- |
| `_item` | 输入 / 输出 | 编辑时传入原始字符串；添加时为空。确认后返回要写入列表的完整字符串（不是仅显示标题）。 |
| `_index` | 输入 | 从 0 开始：编辑为当前项索引，添加为预定插入位置。 |
| `_isNew` | 输入 | 添加为 `true`，编辑为 `false`，便于共用一个子程序。 |
| `_confirmed` | 输出 | 为 `true` 时才应用 `_item`；`false` 表示取消本次单项编辑。 |

只有 `_confirmed` 为 `true` 时才会改列表工作副本。单项取消、子程序异常或关闭列表窗口，都不会提交那一次修改。编辑进行中会暂时禁用添加、编辑、删除、排序、重置和「完成」，「取消」仍可用。列表仍须点窗口上的「完成」才写回动作变量；点「取消」会丢弃本次窗口内的全部改动。

子程序里若打开多字段表单，会以列表窗口为 Owner。不要在子程序里直接把整份列表持久化到动作状态或文件；等主动作根据「是否确认」再保存，避免用户取消列表窗口后仍留下副作用。

使用自定义子程序的动作需要 **2.2.22 或更高版本**；分享时请注明最低版本。回退与兼容见[体验前必读](/important-notice#2222-列表项自定义子程序与提示消息默认设置)。
...
- 2.2.22 增加「添加项子程序」「编辑项子程序」，可用子程序（如多字段表单）自定义添加与编辑。
