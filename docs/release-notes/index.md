...
官网版本记录目前写到 2.1.26，完整条目以那里为准。安装包也从 [https://getquicker.net/V2](https://getquicker.net/V2) 下载，不要使用仍提供 1.x 的 `/Download` 页。

本站补充的是安装、迁移、功能说明和相对 1.x 的结构变化，例如 [V2 重要变化](/v2/what's-new/)。文档会随版本抽查修订，但不会在每次发版时完整复述官网 changelog。

2.1.26 的用户可见补充集中在 OCR 结果窗的翻译页签、代码编辑器的区域折叠、**屏幕找图/找色/找字** 的指定图片 / 指定窗口细节、网络子程序定义预览，以及动作选择器入口恢复。本站只补操作上会遇到的差异，完整版本记录仍见官网。
