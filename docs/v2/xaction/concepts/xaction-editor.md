...
2.2.21 起，动作设计器与表达式等代码编辑框的补全默认使用 **在线补全服务**，从而减小安装包并降低 Quicker 进程占用的内存。需要离线编写，或不希望把代码发到服务器时，可在 **设置 → 动作设计 → 代码补全服务** 中安装本地补全组件，勾选 **使用本地补全服务** 后保存。

注意：

- 「使用本地补全服务」默认关闭；开启前须先完成本地组件安装。
