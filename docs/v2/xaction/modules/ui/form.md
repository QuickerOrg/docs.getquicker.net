...
2.2.7 起，若某个字段绑定了不存在的变量，Quicker 会在打开表单前明确提示缺失变量，而不是进入表单后再异常中断。2.2.8 起，多字段表单中对小数连续加减时，不再出现多余尾数。2.2.13 起，可使用单选按钮列表，并为文本字段启用表达式校验。
...
**验证表达式**：文本框的校验规则。默认按正则表达式处理；2.2.13 起可勾选“使用表达式校验”，输入 Quicker 表达式并获得补全。离开字段时会显示校验错误；重置或改为合法值后提示会清除。
