...
- Agent Skills 权威目录为 **`.agents/skills/`**（Cursor / Codex 均可发现）。`.cursor/skills/` 仅保留兼容桩，勿在桩里维护正文。单篇文档改写入口见 `.agents/skills/docs-page-rewrite/`（**正文 + 换图 + 示例动作**；无图也要跑 `body-rewrite`）。截图→组件流程见 `.agents/skills/docs-img-to-component/`（含 `focusKeys`、**尽量还原场景**；OCR 自动探测，不可用则读 `data/screenshot-replace/ocr/` 或 agent 看图）。分享动作链接→步骤定义见 `.agents/skills/docs-example-action/`。从 Quicker 主仓抽 UI/样式到文档预览见 `.agents/skills/docs-port-quicker-ui/`。改写默认一次一篇，做完后问用户指定下一篇。
