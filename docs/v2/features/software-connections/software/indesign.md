...
18.5–21.x；已有 2026（21.0）真机记录。面向 Windows x64。支持范围不等于所有小版本均已真机测试；实际可安装的版本组合以安装器检测与选择结果为准。
...
启动脚本随 InDesign 自动运行，不需要打开面板。多个版本或语言配置可能有不同 Startup Scripts 目录，以安装器检测到的实际目录为准。
