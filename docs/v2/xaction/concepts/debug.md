...
2.2.17 起，动作输出的日志也可从 **工具 → 运行记录 → 动作日志** 查看，支持选择动作与调用链、筛选级别和关键字、翻页及导出。原独立“动作日志”菜单已合并到这里。手动输出的日志与调试运行的步骤输入输出不同，不会自动包含所有变量值。

2.2.11 起，不重新执行动作也可以从 [标题栏「更多」→「工具」→「动作运行与触发记录」](/v2/features/tools#动作运行与触发记录) 查看已记录的结果和触发来源。可诊断的失败动作会自动保留最近步骤；它不包含完整输入输出或变量值，不能替代下文的调试运行。

需要重现并检查参数值时再选择调试运行；会产生外部副作用的动作请先确认可安全重跑。
