...
在命令工具中，可浏览 Bridge 提供的命令，以及当前宿主 `Mastercam.ft` 中的界面命令：按来源、分类或名称筛选，立即试运行，或 **生成动作** 后粘贴到 Quicker 面板。2.2.17 起，命令工具还支持筛选和查找**第三方 FT** 命令，便于在自定义 Function Table 中定位目标。
