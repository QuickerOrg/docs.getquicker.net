...
2024–2026；已有 2026.3 真机记录。面向 Windows x64。支持范围不等于所有小版本均已真机测试；实际可安装的版本组合以安装器检测与选择结果为准。
...
重启后后台插件自动加载。多个年份有各自偏好目录，应核对安装器列出的目标；Commandline、c4dpy、Team Render 配置不是桌面版目标。
