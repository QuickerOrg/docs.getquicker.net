...
2.2.17 起，从步骤窗口的“变量”按钮打开时，左侧变量列表默认展开，便于集中管理；在右侧变量列表双击某个变量时，默认收起列表，集中编辑该变量。可用窗口中的列表开关切换视图。
...
2.2.11 起，可选“指定类型”，填写必需的 C# 类型，例如 `int`、`System.DateTime` 或 `System.Collections.Generic.List<string>`。初始化及后续赋值会检查实际值是否可赋给目标类型，不做隐式转换：`int` 接受整数 `123`，不接受文本 `"123"` 或 `long` 值。检查失败时保留原值并提示变量名和实际类型。

- 空默认值使用 `default(T)`；引用类型及可空类型可以为 null，非可空值类型不能被赋为 null。
- 默认值以 `$=` 开头时按表达式计算，其它文本仍是字符串。初始化按变量声明顺序进行，只能引用已初始化的变量。
- 编辑时允许保存语法合法但本机缺少依赖的类型；运行时无法解析该类型会报错，不会自动退回动态对象。
- 指定类型不支持状态保存，也不能生成 V1 分享制品。含此类变量的主程序或子程序请使用新版 V2 运行。

“动态对象”的可选 C# 类型只用于补全提示，不限制或转换运行时的值。需要接受任意对象时仍选动态对象，不要仅为补全提示改成指定类型。

## 在步骤窗口中编辑变量

2.2.11 起，步骤编辑窗口的变量菜单可以直接新增或编辑变量；空列表会直接进入创建。改名会同步更新所属程序的引用，包括局部子程序。只读视图可查看已有定义，不能创建或保存。

变量修改单独确认：已经确认的变量修改，不会因随后取消步骤编辑而撤销。请在变量窗口确认名称、类型和影响后再保存。
