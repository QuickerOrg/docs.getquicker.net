...
- **匹配子窗口详情**（2.2.19 起）：查找多个子窗口时输出词典。键为子窗口句柄，值包含 `Handle`、`ClassName`、`Title`、`ParentHandle`、位置、可见性、层级和结果序号。`Depth` 从直接子窗口的 `1` 开始，`Index` 从匹配结果的 `0` 开始。需要根据类名、标题或层级继续处理时，优先使用该输出，而不是只读取“句柄 → 标题”的简化词典。
