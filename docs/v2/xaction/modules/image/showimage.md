...
2.1.23 起，由新版贴图步骤生成的 HTML 或公式贴图也会以普通图片窗口显示，因此可继续拖移、显示阴影，并可在 OCR 完成后选中文字复制。如果剪贴板图片携带 Quicker 或 PixPin 的位置数据，独立贴图步骤可贴回原处；本 **显示图片** 模块仍按下方 **显示位置** 和 **位置坐标** 参数决定窗口位置。
