...
2.1.23 起，由新版贴图步骤生成的 HTML 或公式贴图也会以普通图片窗口显示，因此可继续拖移、显示阴影，并可在 OCR 完成后选中文字复制。2.1.27 起，HTML、公式或剪贴板贴图窗口已经显示后，不再因后续未生成位图提示而误弹“未产生位图”。2.1.28 起，贴图右键菜单「图像处理」可用 **抠出主体（透明背景）**，在本机完成主体抠图后继续保存或贴图。如果剪贴板图片携带 Quicker 或 PixPin 的位置数据，独立贴图步骤可贴回原处；本 **显示图片** 模块仍按下方 **显示位置** 和 **位置坐标** 参数决定窗口位置。
...
- 2.1.27：修复 HTML、公式或剪贴板贴图窗口已显示后仍可能提示“未产生位图”的问题。
- 2.1.28：贴图右键菜单「图像处理」新增「抠出主体（透明背景）」。
