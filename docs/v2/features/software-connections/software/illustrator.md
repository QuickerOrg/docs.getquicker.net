...
Illustrator 2023（27.0）及以上；已有 2026（30.3）真机记录。面向 Windows x64。2.2.13 将安装检测的最低版本扩展到 2023。支持范围不等于所有小版本均已真机测试；实际可安装的版本组合以设置页检测与选择结果为准。

## 安装前准备与首次启用

安装机制：**用户级签名 CEP 扩展**。

退出 Illustrator 后通过设置页软件卡片安装。正式包包含经过签名验证的 CEP 扩展，不需要开启 PlayerDebugMode 或加载未签名源码目录。
