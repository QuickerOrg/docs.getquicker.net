...
[官网 2.1.25 记录](https://getquicker.net/V2/Versions)中，本模块的查找范围新增了 **指定窗口** 和 **指定图片**：前者适合在被遮挡的目标窗口中查找，后者适合在本地图片里查找，不必先把内容显示到屏幕上。当前仓库的 `data/xaction` 还没有同步这两个选项的枚举值和后续参数 Key，因此本页只说明能力，不写参数 Key、示例值或可导入的配置片段。
...
**查找范围**：主屏幕、当前窗口、坐标范围、所有屏幕。2.1.25 起还可选择 **指定窗口** 或 **指定图片**；这两个选项的具体参数以当前 Quicker 编辑器和官网版本记录为准。注意 **当前窗口** 是动作运行时的当前前台窗口，不等同于新增的 **指定窗口**。
