---
title: "屏幕找图/找色/找字"
description: "在屏幕、窗口或图片中查找图片、颜色或文字，返回匹配点坐标。"
...
在屏幕、窗口或图片中查找指定图片、颜色或文字，返回匹配点坐标。常用来定位按钮、菜单后再点。
...
[官网 2.1.25 与 2.1.26 记录](https://getquicker.net/V2/Versions)中，本模块的查找范围新增并继续完善了 **指定窗口** 和 **指定图片**：前者适合在被遮挡的目标窗口中查找，后者适合在本地图片里查找，不必先把内容显示到屏幕上。当前仓库的 `data/xaction` 还没有同步这两个选项的枚举值和后续参数 Key，因此本页只说明能力，不写参数 Key、示例值或可导入的配置片段。

升级注意：如果你在 2.1.25 里已经把查找范围设为 **指定图片**，升级到 2.1.26 后请重新打开该步骤编辑并保存一次，让目标图片设置按新版格式更新。
...
**查找范围**：主屏幕、当前窗口、坐标范围、所有屏幕。2.1.25 起还可选择 **指定窗口** 或 **指定图片**；2.1.26 起，指定图片的目标可来自图片路径或图片变量，指定窗口可选择返回窗口相对坐标。这两个选项的具体参数以当前 Quicker 编辑器和官网版本记录为准。注意 **当前窗口** 是动作运行时的当前前台窗口，不等同于新增的 **指定窗口**。
...
查找范围为 **指定窗口** 时，2.1.26 起可在步骤设置中选择是否返回窗口相对坐标。需要把结果交给窗口内部处理、或窗口可能移动时，优先考虑窗口相对坐标；需要直接移动系统鼠标时，仍要确认后续步骤期望的是屏幕坐标还是窗口坐标。
...
- 2.1.26：指定图片目标支持图片路径或图片变量；指定窗口可选择返回窗口相对坐标。2.1.25 中已使用指定图片的动作升级后需要重新打开步骤编辑一次。
- 2.1.25：查找范围新增指定窗口、指定图片。当前结构化模块数据尚未同步对应参数。
