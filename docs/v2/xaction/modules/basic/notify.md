...
默认出现在屏幕底部居中。2.2.22 起，可在设置里改全局默认位置和停留时间，见下方[默认位置与停留时间](#默认位置与停留时间)。
...
**显示位置**：底部居中、四角、顶/底居中。仅成功 / 信息 / 告警 / 错误。2.2.22 起，步骤还可选择跟随设置中的全局默认位置；未单独指定时使用该默认。
...
**保持秒数**：`0` 用默认时长，`-1` 一直显示，或填秒数。2.2.17 起按设置可保持 **1 秒**；更早版本界面与元数据常见范围为 2～30 秒。2.2.22 起，「默认时长」可在设置中配置；步骤填 `0` 时跟随该全局默认（带按钮的提示仍至少保留约 10 秒互动时间）。
...
2.2.22 起，在 **设置 → 模块功能选项** 可单独配置提示消息的 **默认显示位置** 和 **默认停留时间**，并随账号同步。默认停留时间为 `0` 时，仍按消息长度自动估算显示时长。

单条提示若在步骤或自动化脚本里单独指定了位置或时长，仍使用该条自身设置，不改全局默认。脚本未指定 placement 时跟随全局默认；显式指定底部居中等位置时仍固定在该位置。

旧客户端不认识这两项设置。回退或混用旧版时，若在旧版保存设置并同步，可能把默认位置和停留时间重置；建议先记录当前值，把相关设备升级后再改。详见[体验前必读](/important-notice#2222-列表项自定义子程序与提示消息默认设置)。
