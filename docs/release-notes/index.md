...
本站功能说明已按 2.2.21 补充，新增了[代码补全服务](/v2/xaction/concepts/xaction-editor#代码补全服务)（在线默认、可选本地组件）、[文本窗口记住位置和大小](/v2/xaction/modules/showtext#记住上次位置和大小)，以及[动作面板共用入口与删除关联位置](/v2/features/action-panel/usage#复制后同时添加与删除关联)等说明。版本是否已开放下载、完整条目及安装包校验值，以官网为准。
...
2.2.21 补充了 [代码补全服务](/v2/xaction/concepts/xaction-editor#代码补全服务)改为在线提供并可在「设置 → 动作设计」安装本地组件、[文本窗口「记住上次位置和大小」](/v2/xaction/modules/showtext#记住上次位置和大小)，以及[动作面板复制后「同时添加到这里」与删除时显示场景关联位置](/v2/features/action-panel/usage#复制后同时添加与删除关联)。在线补全会向服务器发送编辑中的代码等信息，见 [体验前必读](/important-notice#2221-代码补全服务)；完整条目仍以[官网版本记录](https://getquicker.net/V2/Versions)为准。
