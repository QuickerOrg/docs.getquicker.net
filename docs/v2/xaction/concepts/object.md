...
2.2.11 起，动态对象可填写 C# 类型，例如 `Newtonsoft.Json.Linq.JToken`，帮助表达式补全属性和方法。该提示不转换对象，也不检查每次赋值是否属于该类型；留空时保持原行为。

需要运行时限制类型时，应选择独立的“指定类型”。两者的默认值、状态保存和分享限制不同，见 [指定 C# 类型](/v2/xaction/concepts/variables#指定-c-类型)。
