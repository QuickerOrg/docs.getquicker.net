...
2.1.27 的用户可见补充集中在浏览器列表提取与表单填写、OCR 结果窗耗时显示 / 同步滚动 / 冷启动预览、链接动作标题图标说明实时跟随目标动作、等待按键忽略模拟鼠标按键、下载与 HTTP 请求重定向兼容，以及快速截图和获取窗口信息可选中透明顶层弹窗。本站只补操作上会遇到的差异，完整版本记录仍见官网。
