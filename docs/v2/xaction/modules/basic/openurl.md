...
用系统或指定浏览器打开网址。2.2.13 起，“网址”参数可每行填写一个目标，一次打开多个网址、文件或文件夹。要运行程序或指定命令行参数，用 [运行或打开](/v2/xaction/modules/run)。
...
**网址**：要打开的地址。2.2.13 起支持多行；每行可以是网址、文件或文件夹，空行和不存在的本地目标会被跳过。
