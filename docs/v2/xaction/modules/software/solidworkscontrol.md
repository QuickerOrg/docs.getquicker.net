...
quickerDocKey: "xaction/module/sys:solidworkscontrol"
comments: true
...
使用前先[安装并连接 SOLIDWORKS Bridge](/v2/features/software-connections/software/solidworks)。本模块用于文档与选择、特征和配置、自定义属性、装配体和工程图、质量属性、宏与脚本；根据任务选择对应操作，或发送当前实例支持的自定义命令。

## 当前模块定义

<XActionModuleMeta moduleKey="sys:solidworkscontrol" />
...
文档与选择、特征和配置、自定义属性、装配体和工程图、质量属性、宏与脚本是常见使用方向。选择操作后填写对应字段；字段名称、默认值、枚举和条件以上方当前模块定义为准。

自定义调用选择 **[属性] 列出自定义属性**，从命令工具复制实际命令名与参数模板。参数 JSON 必须是对象，路径中的反斜杠需要转义。不要把其他软件的参数直接套用到本模块。
...
回到已连接的 SOLIDWORKS 窗口，再通过面板或配置好的快捷键触发动作。
...
- [SOLIDWORKS 安装与使用指南](/v2/features/software-connections/software/solidworks)
