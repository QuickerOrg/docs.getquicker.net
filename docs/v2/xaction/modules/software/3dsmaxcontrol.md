---
title: "3ds Max控制"
description: "通过 3ds Max Python 插件发送命令；仅有一个在线连接时直接使用，多个连接时按动作触发窗口定位。"
slug: "/v2/xaction/modules/3dsmaxcontrol"
sidebar_label: "3ds Max控制"
sidebar_position: 10
quickerDocKey: "xaction/module/sys:3dsmaxcontrol"
comments: true
moduleKey: "sys:3dsmaxcontrol"
docStatus: "reviewed"
...
使用前先[安装并连接 3ds Max Bridge](/v2/features/software-connections/software/3ds-max)。本模块用于场景与选择集查询、对象和图层整理、Action 搜索与调用、MAXScript/Python；根据任务选择对应操作，或发送当前实例支持的自定义命令。

## 当前模块定义

<XActionModuleMeta moduleKey="sys:3dsmaxcontrol" />
...
场景与选择集查询、对象和图层整理、Action 搜索与调用、MAXScript/Python是常见使用方向。选择操作后填写对应字段；字段名称、默认值、枚举和条件以上方当前模块定义为准。
...
回到已连接的 3ds Max 窗口，再通过面板或配置好的快捷键触发动作。
...
- [3ds Max 安装与使用指南](/v2/features/software-connections/software/3ds-max)
