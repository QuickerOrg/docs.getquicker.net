...
2.1.29–2.2.0 的用户可见补充集中在新版主窗口侧边栏快捷按钮、暂存区「新建动作 / 用 AI 写」和「保留到场景」、贴图通用 / 人像抠图、自动化脚本面向窗口与区域的 OCR / 找图 / 找色和相对点击、窗口操作的贴边自动隐藏 / 类似窗口关闭与切换，以及步骤编辑器可拉伸或贴靠到完整屏幕高度。本站只补操作上会遇到的差异，完整版本记录仍见官网。
