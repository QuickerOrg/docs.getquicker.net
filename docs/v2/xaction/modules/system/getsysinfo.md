...
将“Quicker版本”输出保存到数字变量，例如 `qkVersion`，再用条件步骤比较数字：

- 判断是否为 2.0.0 或更高版本：`qkVersion >= 2000000`。
- 判断是否为 2.2.10 或更高版本：`qkVersion >= 2002010`。

上面表示比较关系；在条件步骤中选择该变量、比较方式和对应数字即可。不要把版本当成小数 `2.2` 或按字符串大小比较。这里读取的是 Quicker 客户端版本，不是动作的修订号。

版本判断适合处理两版都能读取的动作中的运行差异，不能让 V1 识别 V2 专属模块或结构。分享检查会遍历条件两侧和子程序，即使 V2 专属步骤不会在 V1 执行，也可能阻止生成 V1 内容。维护方式见[动作作者迁移指南](/v2/migration/action-authors)。
