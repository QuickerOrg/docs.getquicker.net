---
title: "Blender控制"
description: "通过 Blender Python Extension 发送命令；仅有一个在线连接时直接使用，多个连接时按动作触发窗口定位。"
slug: "/v2/xaction/modules/blendercontrol"
sidebar_label: "Blender控制"
sidebar_position: 90
quickerDocKey: "xaction/module/sys:blendercontrol"
comments: true
moduleKey: "sys:blendercontrol"
docStatus: "reviewed"
...
使用前先[安装并连接 Blender Bridge](/v2/features/software-connections/software/blender)。本模块用于场景、对象和集合、选择与视图、修改器与材质、渲染及 Python；根据任务选择对应操作，或发送当前实例支持的自定义命令。

## 当前模块定义

<XActionModuleMeta moduleKey="sys:blendercontrol" />
...
场景、对象和集合、选择与视图、修改器与材质、渲染及 Python是常见使用方向。选择操作后填写对应字段；字段名称、默认值、枚举和条件以上方当前模块定义为准。
...
回到已连接的 Blender 窗口，再通过面板或配置好的快捷键触发动作。
...
- [Blender 安装与使用指南](/v2/features/software-connections/software/blender)
