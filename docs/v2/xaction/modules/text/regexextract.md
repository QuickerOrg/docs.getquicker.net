...
metadataHash: "5035a69b064f7c3eb6cc8489938774d50d25920a3103d4d0a743b37149bbaf02"
legacyDocId: 2115456
...
| --- | --- | --- | --- | --- | --- | --- | --- |
| `getGroup` | 提取方式 | `Enum` | 0 | 是 | `Input` |  | 输出的内容根据提取内容有所不同，请参考模块文档。 |
| `data` | 输入 | `Text` |  | 是 | `UseVarOrInput` |  | 要提取内容的文本 |
| `pattern` | 正则表达式 | `Text` |  | 是 | `UseVarOrInput` |  | 用于提取内容的正则表达式 |
| `ignoreCase` | 忽略大小写 | `Boolean` | false | 否 | `UseVarOrInput` |  | 不区分英文大小写 |
| `singleLine` | 单行模式 | `Boolean` | false | 否 | `UseVarOrInput` |  | 此模式下"."能匹配任意字符，包括换行符。(否则匹配除了\n外的任意字符) |
| `multiLine` | 多行模式 | `Boolean` | false | 否 | `UseVarOrInput` |  | 此模式下^和$可以分别匹配行首和行尾。(否则匹配输入内容的开始和结束) |
| `rightToLeft` | 从右向左 | `Boolean` | false | 否 | `UseVarOrInput` |  | 从右向左查找匹配内容 |
| `stopIfFail` | 失败后中止动作 | `Boolean` | true | 否 | `Input` |  | 操作失败后，是否停止后续动作的执行。 |
...
| `matches` | 所有匹配列表 | `List` |  | 返回所有的匹配项 |
| `match1 ` | 匹配1 | `Any` |  | 第1个匹配项的值 |
| `match2 ` | 匹配2 | `Any` |  | 第2个匹配项的值 |
| `match3 ` | 匹配3 | `Any` |  | 第3个匹配项的值 |
| `match4 ` | 匹配4 | `Any` |  | 第4个匹配项的值 |
| `match5 ` | 匹配5 | `Any` |  | 第5个匹配项的值 |
| `matchObj` | Match对象 | `Object` | 仅：1 | 首个匹配的原始的C#语言Match对象，可以在表达式中使用 |
| `matchesCollection` | Matches集合 | `Object` | 仅：0, 2 | 所有匹配的原始Match对象集合(MatchCollection)，可以在表达式中使用 |
