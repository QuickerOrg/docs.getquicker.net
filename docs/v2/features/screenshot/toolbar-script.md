...
2. **开动作（确认关窗）**：`act('动作名或ID')`，或 `act('动作名或ID', { title, icon, post })`
3. **最后**：`return [ …按钮… ]`

- `sp`：第二参是子程序输入变量对象（键名与变量同名）；第三参才是按钮标题 / 图标 / 后处理。当前图默认自动带上；若图片变量不叫 `image`，写成 `photo: $image`（不要写成字符串 `'$image'`）。
- `act`：只要改标题 / 图标时，把选项直接放在第二参：`act(id, { title: '发送' })`。带启动参数时用 `act(id, '参数')`、`act(id, '参数', { title })`，或 `act(id, { title, param: '参数' })`。启动参数是字符串。不要写占位 `null`。

后处理写在 **`post: { … }`** 里（例如 `clipboard: true`）。需要 if / 多步时，再写带 `async run` 的完整按钮（见下文）。
...
- `act(...)` 第一参换成动作 ID（或名称）。若要带启动参数：`act(id, '可选启动参数', { title: '发送' })`。
