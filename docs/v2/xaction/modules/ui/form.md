...
2.2.7 起，若某个字段绑定了不存在的变量，Quicker 会在打开表单前明确提示缺失变量，而不是进入表单后再异常中断。2.2.8 起，多字段表单中对小数连续加减时，不再出现多余尾数。2.2.13 起，可使用单选按钮列表，并为文本字段启用表达式校验。2.2.14 起，常用扩展选项可直接在结构化界面编辑，仍保留高级原始扩展设置入口；校验规则统一在同一输入框中设置。2.2.15 起，单选列表与字段标题的对齐已修复。2.2.22 起，多选列表的已选标签也会显示选项图标，见[多选列表已选标签图标](#多选列表已选标签图标)。
...
**验证表达式**：文本框的校验规则，统一在同一输入框中设置。默认按正则表达式处理；以 `$=` 前缀表示表达式校验（2.2.14 起不再用「勾选使用表达式校验」），并获得表达式补全。历史上以 `$=` 开头的正则会被当成表达式。离开字段时会显示校验错误；重置或改为合法值后提示会清除。

**单选按钮列表**：2.2.13 起可把候选项直接显示为单选按钮。扩展设置可用 `radio_direction:horizontal` 改为横向排列，或用 `radio_columns:3` 指定每行列数（1–12）；不设置时自动换行。2.2.15 起，单选列表与字段标题的对齐已修复。

### 多选列表已选标签图标

2.2.22 起，多选下拉（多选列表）里已经选中的标签会与下拉项一样显示选项图标，而不只显示文字名称。选项本身未配置图标时，标签仍只显示名称。
...
**扩展设置**：控制控件行为。2.2.14 起，常用扩展选项可直接在结构化界面编辑；仍可通过高级入口编辑原始扩展设置（每行一条指令）。
