...
2.2.20 起，定义输入参数时，代码样式的输入可选择代码语言；参数较多时，也可使用高级分组整理调用界面。它们只影响调用者看到和编辑参数的方式，不改变变量名、输入 / 输出标记或传值规则。
...
2.1.29 起，新建公共子程序后可以直接按 `Ctrl+S` 保存，不必先切换到其它位置再回来保存。
...
可导入为动作内再改，或在「共享」标签搜索后直接拖进步骤。右键可查看定义或导入。2.1.26 起，拖进步骤列表后的网络共享子程序步骤也能右键 **查看定义**，预览的是当前步骤引用的版本；要修改时仍需导入或转成动作内子程序。
