...
2.2.20 起，编辑子程序输入参数时，代码样式的输入可选择代码语言，参数界面也支持高级分组，便于把复杂子程序的选项按用途收纳。这里是界面配置能力，不改变输入变量的名称或传值方式；具体字段请以客户端变量属性窗口为准。
...
2.1.26 起，已经拖进步骤列表的网络共享子程序步骤，也可以从右键菜单点 **查看定义**。这会预览当前步骤引用的子程序版本，方便确认内部步骤和变量；如果要改定义，仍需先导入或转换为动作内子程序。
...
- 2.2.20：子程序输入参数支持代码语言选择和高级分组。
- 2.1.28：调试结果支持定位共享子程序步骤。
- 2.1.26：网络共享子程序步骤右键菜单新增「查看定义」，可预览当前引用版本。
- 20250606 修改描述错误。
