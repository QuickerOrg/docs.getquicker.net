...
2.2.21 起，显示窗口时可在高级参数中开启 **记住上次位置和大小**：再次运行同一动作时，会按“文本窗口标识”恢复上次关闭前的位置和尺寸。默认关闭；全屏或最大化的位置类型不使用记忆位置。需要固定初始布局时保持关闭，并继续用上方的「窗口位置类型」和「窗口尺寸/位置」控制。记忆值与“自动保存到状态”的文本内容分开保存。

参数表待 `data/xaction` 同步后由模块预览自动带出；完整版本记录见[官网](https://getquicker.net/V2/Versions)。
